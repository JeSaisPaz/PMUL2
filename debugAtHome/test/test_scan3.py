...
def scan_frame(cam):
    ...
    try:
        ...
        # Décodeur QR OpenCV (plus robuste que pyzbar sur images caméra)
        ...
        def try_decode(img):
            """Tente de décoder avec le QRCodeDetector OpenCV."""
            data, points, _ = detector.detectAndDecode(img)
            if data and points is not None:
                return data, points
            return None, None

        # 1. Essai direct sur le gris
        qr, points = try_decode(gray)

        # 2. CLAHE si raté
        if not qr:
            enhanced = clahe.apply(gray)
            qr, points = try_decode(enhanced)

        # 3. Zoom x2 + CLAHE si encore raté
        if not qr:
            zoomed   = cv2.resize(gray, (w*2, h*2), interpolation=cv2.INTER_CUBIC)
            enhanced2 = clahe.apply(zoomed)
            qr, points = try_decode(enhanced2)
            if points is not None:
                points = points / 2  # ramene les coords a la taille originale

        # 4. Seuillage adaptatif en dernier recours
        if not qr:
            binary = cv2.adaptiveThreshold(clahe.apply(gray), 255,
                         cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
            qr, points = try_decode(binary)

        if not qr or points is None:
            _set_preview(frame, "RATE - QR non detecte")
            return None, 0, 0, 0

        # Bounding box depuis les 4 coins détectés par OpenCV
