...
def init_camera():
    ...
    try:
        ...
        cam.set_controls({"AwbEnable": False, "ExposureTime": 9000,
                          "AnalogueGain": 1.0, "ColourGains": (1.3, 1.7), "Saturation": 0.9})
        time.sleep(1.0)
...
def scan_frame(cam):
    ...
    try:
        ...
        if not hs: return qr, 0, 0, 0
        return qr, int(sum(hs)/len(hs)), int(sum(ss)/len(ss)), int(sum(vs)/len(vs))
    except Exception as e:
