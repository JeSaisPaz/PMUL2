...
def init_camera():
    ...
    try:
        ...
        cam.set_controls({
            ...
            "Saturation":   1.0,    # Neutre - ajuste entre 0.8 (moins) et 1.4 (plus)
            "Sharpness":    1.5,
        })
        time.sleep(1.0)
...
# Nombre de captures max si le QR rate (chaque tentative = nouvelle photo)
MAX_SCAN_RETRIES = 4

def _decode_qr(frame, w, h):
    """
    Tente de décoder un QR code depuis un frame BGR via OpenCV.
    Essaie 4 preprocessings différents.
...
def scan_frame(cam):
    ...
    Tente MAX_SCAN_RETRIES captures si le QR n'est pas détecté.
    """
    if cam is None:
    ...
    try:
        ...
        for attempt in range(1, MAX_SCAN_RETRIES + 1):
            ...
            if attempt < MAX_SCAN_RETRIES:
                time.sleep(0.15)  # courte pause entre captures

        if not qr or points is None:
            _set_preview(frame, f"RATE apres {MAX_SCAN_RETRIES} tentatives")
            return None, 0, 0, 0

        # Calcul HSV autour du QR code
...
def handle_scan_needed(st, cam):
    ...
    if qr is None:
        print("  [!] QR non detecte - envoi scan vide au backend")
        qr  = ""   # chaine vide = scan rate, le backend decidera
        hue = 0
        sat = 0
        val = 0
